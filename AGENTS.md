...
- Env access is centralized in `src/config/env.ts` (lazy proxy over edge bindings, `Bun.env`, `process.env`); read new vars there, not inline. New gate vars: `PLUNK_API_KEY` (non-blank arms the gate; blank/unset is open access, the self-host default — there is intentionally no separate flag), `SESSION_SECRET`, `PLUNK_FROM_EMAIL`, `PLUNK_TEMPLATE_ID` (optional), `PLUNK_API_URL` (optional, defaults to `https://next-api.useplunk.com`). `IDHS_API_KEY_BETA` stays dormant (read, never used); do not build auth logic on it.
...
- Worker secrets (names only, values never in repo; set with `bunx wrangler secret put`): `PLUNK_API_KEY` (its presence arms the gate), `SESSION_SECRET`, `PLUNK_FROM_EMAIL` (unless the template defines a sender), optional `PLUNK_TEMPLATE_ID`. Plunk also needs a verified sender domain in its dashboard. Kill-switch is blanking or deleting the `PLUNK_API_KEY` secret: open access without redeploy.
