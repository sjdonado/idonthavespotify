...
- Tidal is parse-only: its search API needs a portal-granted entitlement that was refused (`invalid_scope` on the token, `INVALID_RESOURCE_ID` on every query), and its search pages sit behind a bot wall. Do not re-add an outbound Tidal adapter without portal access.
