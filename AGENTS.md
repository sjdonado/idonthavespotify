...
- Env access is centralized in `src/config/env.ts` (lazy proxy over edge bindings, `Bun.env`, `process.env`); read new vars there, not inline. New gate vars: `GATE_ENABLED` (`1`/`true` enables; unset/off is open access, the self-host default), `SESSION_SECRET`, `PLUNK_API_KEY`, `PLUNK_FROM_EMAIL`, `PLUNK_TEMPLATE_ID` (optional), `PLUNK_API_URL` (optional, defaults to `https://api.useplunk.com`).
- `tests/utils/shared.ts` reads `ENV` at module-eval time, which freezes the lazy env cache for the whole test file. Any test file that sets `process.env` per-file (e.g. the gate suite) must not import it; inline the endpoint builder instead.
...
## Workers operations (public demo only)
...
- Worker secrets (names only, values never in repo; set with `bunx wrangler secret put`): `GATE_ENABLED=1`, `SESSION_SECRET`, `PLUNK_API_KEY`, `PLUNK_FROM_EMAIL` (unless the template defines a sender), optional `PLUNK_TEMPLATE_ID`. Plunk also needs a verified sender domain in its dashboard. Kill-switch is `GATE_ENABLED=0`/unset: open access without redeploy.
- GitHub deploy secrets: `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, optional `DEMO_URL` (smoke target, defaults to `https://idonthavespotify.donado.co`).
- Edge deltas vs self-host: platform `fetch` instead of TLS impersonation (guarded sources may answer differently; Spotify metadata resolves via `__NEXT_DATA__` embed pages on both runtimes; Apple Music resolves via the same-host catalog chain with no audio preview; Tidal search currently 400s from edge egress and degrades to a logged miss, self-host unaffected), per-isolate in-memory cache (service-guard budgets stay the shared quota protection), no URL shortener (share links are always plain app URLs), no per-IP limiting in the app, and platform CPU and memory limits.
