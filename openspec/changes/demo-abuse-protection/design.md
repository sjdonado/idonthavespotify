...
- **No signup page: gate UI inline** in the home view (email field where the search bar is; OTP field replaces it on send). Session = signed HttpOnly cookie, the only credential; no bearer scheme, so third-party API clients stay a self-hosted story where the gate is off. Gate placement: middleware-style wrapper around the four search handlers in `createRoutes` so both runtimes inherit it; static assets and `/api/status` stay open (status is needed for monitoring and leaks nothing).
...
- [Risk] Gate blocks the Raycast extension on the public demo → Accepted: the extension targets self-hosted instances, where the gate is off; kill-switch covers emergencies.
